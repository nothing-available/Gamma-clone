...
import pic from 'public/pic_1.jpg';
...
function ThemeCard({ content, controls, description, theme, title, variant }: Props) {
  const variants = {
    left: {
      hidden: { opacity: 0, x: '-50%', y: '-50%', scale: 0.9, rotate: 0 },
      visible: {
        opacity: 1, x: '-25%', y: '-25%', scale: 0.9, rotate: -10,
      ...
      >
        <div className='flex flex-col md:flex-row'>
          ...
          <div className='relative w-full md:w-1/2 h-80 md:h-auto overflow-hidden rounded-r-lg'>
            <Image
              src={pic}
              alt='Theme illustration' // Improved with descriptive alt text
              fill
